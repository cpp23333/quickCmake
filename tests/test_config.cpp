...
sylar::ConfigVar<float>::ptr g_fooat_value_config =
...
sylar::ConfigVar<std::list<int>>::ptr g_list_config =
    sylar::Config::Lookup("system.int_list", std::list<int>{1, 2}, "system int vec");
...
void test_yaml()
{
    YAML::Node root = YAML::LoadFile("/home/cpp/C++/log.yml");
    print_yaml(root, 0);
...
void test_config()
{
    ...
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "before: " << g_fooat_value_config->toString();
...
#define XX_M(g_var, name, prefix)                                                             \
    ...
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "after:  " << g_fooat_value_config->toString();
...
#define XX_PM(g_var, prefix)                                                                               \
    ...
    g_person->addListener(10, [](const Person &old_vale, const Person &new_value)
                          { SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << old_vale.toString() << " -> " << new_value.toString(); });
